...
__global__ void nonAtomicHistogram(unsigned char* input, unsigned int* histogram, int n) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
int main() {
    ...
    printf("Complete the missing code sections in the atomicHistogram kernel.\n\n");
    ...
    h_histogram_non_atomic = (unsigned int*)calloc(HIST_SIZE, sizeof(unsigned int));
    ...
    nonAtomicHistogram<<<gridSize, blockSize>>>(d_input, d_histogram_non_atomic, N);
    cudaDeviceSynchronize();
    
    // Copy results back to host
    cudaMemcpy(h_histogram_atomic, d_histogram_atomic, hist_size, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_histogram_non_atomic, d_histogram_non_atomic, hist_size, cudaMemcpyDeviceToHost);
    ...
    // Cleanup
    free(h_input); free(h_histogram_atomic); free(h_histogram_non_atomic);
    cudaFree(d_input); cudaFree(d_histogram_atomic); cudaFree(d_histogram_non_atomic);
