/*
 * Constant Memory - Complete Exercise File
 * 
 * This file contains exercises for constant memory usage.
 * Complete the TODO sections to learn constant memory programming.
 */
...
// TODO: Declare constant memory for lookup table
// __constant__ float sinTable[TABLE_SIZE];
/* YOUR DECLARATION HERE */

// Kernel using constant memory coefficients
__global__ void polynomialTransform(float *input, float *output, int n) {
    ...
    if (idx < n) {
        ...
        // TODO: Use constant memory coefficients
        // for (int i = 0; i < COEFF_COUNT; i++) {
        //     result += d_coeffs[i] * powf(x, i);
        // }
...
// Kernel using constant memory lookup table
__global__ void lookupTransform(float *input, float *output, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    
    if (idx < n) {
        // TODO: Use constant memory lookup table
        // int tableIdx = (int)(input[idx] * (TABLE_SIZE - 1));
...
// Kernel comparing constant vs global memory
__global__ void constantVsGlobal(float *input, float *output, 
    ...
    if (idx < n) {
        ...
        // TODO: Compute using constant memory
        // for (int i = 0; i < COEFF_COUNT; i++) {
        //     resultConstant += d_coeffs[i] * powf(x, i);
        // }
        
        // Compute using global memory
        for (int i = 0; i < COEFF_COUNT; i++) {
            resultGlobal += globalCoeffs[i] * powf(x, i);
        }
...
int main() {
    printf("=== Constant Memory Exercises ===\n\n");
    ...
    // TODO: Copy sin table to constant memory
